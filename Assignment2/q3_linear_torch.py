...
class Linear(DQN):
    ...
    """
    def initialize_models(self): # called with Linear(env, config)
        ...
        self.q_network = nn.Linear(img_height*img_width*n_channels*self.config.state_history, num_actions, bias=True)
        self.target_network = nn.Linear(img_height*img_width*n_channels*self.config.state_history, num_actions, bias=True)
    ...
    def get_q_values(self, state, network='q_network'):
        """
        Returns Q values for all actions [a tensor]
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        # print(num_actions, gamma)

        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############        
        Q_samp = rewards + gamma * torch.max(target_q_values, dim=1).values * (~done_mask)
        Q_pred = torch.sum(q_values * torch.nn.functional.one_hot(actions.to(torch.int64), num_classes=num_actions), dim=1)
        loss = torch.nn.functional.mse_loss(Q_pred, Q_samp)
        # print("loss printed on line 145; q3_linear_torch.py: ", loss)
        return loss
        ##############################################################
        ######################## END YOUR CODE #######################
